...
import { rankByMetadataPrecedence, type RankedChunk, type RetrievalFacets } from "@/lib/retrieval/metadata-precedence";
import { FACET_LIMITS, sanitizeEmotions, sanitizeLowercaseStrings, toNullable } from "@/lib/retrieval/sanitize-facets";

const MAX_QUERY_EMBEDDING_DIMS = 4096;

export type RetrieveStats = {
  top_k: number;
  blend_ratio: number;
  base_top_k: number;
...
export async function runRetrieval(input: RetrieveInput): Promise<RetrieveResponse> {
  const query = typeof input.query === "string" ? input.query.trim() : "";
  if (!query) throw new Error("`query` (string) is required");
  if (!input.gatekeeper || typeof input.gatekeeper !== "object") {
    throw new Error("`gatekeeper` object is required");
  }

  const qe = input.queryEmbedding;
  if (Array.isArray(qe) && qe.length > MAX_QUERY_EMBEDDING_DIMS) {
    throw new Error(`\`queryEmbedding\` exceeds max dims (${MAX_QUERY_EMBEDDING_DIMS})`);
  }

  // Do not trust facets from the caller: sanitize and cap sizes.
  const gate = input.gatekeeper as unknown as Record<string, unknown>;
  ...
  const facets: RetrievalFacets = {
    ...
    people,
    keywords,
    date_int: typeof input.gatekeeper.date_int === "number" ? input.gatekeeper.date_int : null,
  };

  const topKFinal = clampInt(input.top_k, 12, 1, 50);
  const baseTopK = clampInt(input.base_top_k, 50, topKFinal, 200);
  const matchedTopK = clampInt(input.matched_top_k, 50, topKFinal, 200);
  const blendRatio = clampRatio(input.blend_ratio, 0.7);

  const rewritten_query = pickQueryText(query, { reframed_query: input.gatekeeper.reframed_query ?? null });

  // If gatekeeper explicitly decided retrieval is irrelevant, bail out early to avoid embedding + Pinecone calls.
  if ((gate as Record<string, unknown>).skip_RAG === true) {
    return {
      rewritten_query,
      stats: {
        top_k: topKFinal,
